...
export function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center rounded-md border border-dashed border-border bg-surface/60 px-8 py-16 text-center">
      <span aria-hidden="true" className="text-[34px]">
        {icon}
      </span>
      <h3 className="mt-4 text-[17px] font-bold tracking-tight">{title}</h3>
      <p className="mt-2 max-w-sm text-[13px] leading-relaxed text-fg-muted">{description}</p>
      {action && (
